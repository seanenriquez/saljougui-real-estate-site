...
class dbSeo extends PDOConfig {
  ...
  // load page header tag info
  public function getPageTitle () {

    $propHeaderTag=ucwords($_GET['proptype']);
    
    if (strtolower($_GET['proptype'])=="house") {
      $propHeaderTag="Houses";
    }
    if (strtolower($_GET['proptype'])=="townhouse") {
      $propHeaderTag="Townhomes";
    }
    if (strtolower($_GET['proptype'])=="condo") {
      $propHeaderTag="Condos";
    }
    if (strtolower($_GET['proptype'])=="villa") {
      $propHeaderTag="Villas";
    }

    if (strtolower($_GET['searchtype'])=="residential") {
      $typeHeaderTag="Sale";
    }

    if (strtolower($_GET['searchtype'])=="rental") {
      $typeHeaderTag="Rent";
    }
    
    $this->page_title = "$_GET[area] $propHeaderTag For $typeHeaderTag";
  ...
  public function getSqFt() {
    return number_format($this->row['square_footage']);
  }
  ...
  public function getThumbSmallFn() {
    return "mls/thumbs200/".$this->row['listing_id']."-1.jpg";
  }

  public function getThumbFn() {
    return "/mls/thumb/".$this->row['listing_id']."-1.jpg";
  }

  public function getFrontPicFn() {
    return "/mls/hires/".$this->row['listing_id']."-1.jpg";
  }
...
class dbSeoModel extends dbSeo {
  ...
  public function getNewestProperty() {
    ...
    $sql = 'SELECT * from master_rets_table 
              WHERE property_type = "Residential" 
                AND city REGEXP "(JUNO BEACH|palm beach gardens|jupiter|WEST PALM BEACH)" 
                AND listing_price > 150000 
                AND listing_price <= 1000000
                AND char_length(remarks) > 50
              ORDER BY listing_entry_timestamp DESC
              LIMIT 12';            
  ...
  public function doSearch () {
    ...
    $this->pagination->records_per_page($recs_per_page);
    $this->pagination->method('url');
    $this->pagination->base_url("",false);
    
    $this->rows = array_slice(
      $this->rows,
      (($this->pagination->get_page() - 1) * $recs_per_page),
      $recs_per_page);
  ...
  /******************************************************************************************************************/
    ...
    if ($this->isCity($area)) {
      ...
      $sql_or[] = "city = '$city'";
		...
		switch ($area) {
		  ...
		  case "north-palm-beach":
      case "north palm beach":
		    $city = $this->cityname = "North Palm Beach";
		  break;

		  case "jupiter":
		    $city = $this->cityname = "Jupiter";
		    // $city_number = '210|212|213|215';
		  break;

		  case "west-palm-beach":
      case "west palm beach":
		    $city = $this->cityname = "West Palm Beach";
		    // $city_number = '381|382';
		  break;

		  case "palm-beach-gardens":
      case "palm beach gardens":
		    $city = $this->cityname = "Palm Beach Gardens";
		    //$city_number = '250|251|252|253|355';
		  break;

		  case "singer-island":
      case "singer island":
		    $city = $this->cityname = "Singer Island";
		   // $city_number = '384|385';
		  break;

		  case "tequesta":
          
		    $city = $this->cityname = "Tequesta";
		    //$city_number = '381|382';
		  break;

		  case "juno-beach":
      case "juno beach":
		    $city = $this->cityname = "Juno Beach";
		   // $city_number = '383|384';
		  break;

  
		  case "palm-beach-shores":
      case "palm beach shores":

		    $city = $this->cityname = "Palm Beach Shores";
		   // $city_number = '347|346|345|344|343|341|340';
		  break;

		  case "jupiter-inlet-colony":
		    $this->cityname = "Jupiter Inlet Colony";
		    $city_number = '340|341';
		  break;
          
		  default:
		  	$this->cityname = 'All Palm Beach and Martin Counties';
    ...
    // hack to manually pass a search filter
		if (!empty($type))
			$sql_addition[] = $type;
    ...
    if (strtolower($_REQUEST['searchtype']) == "rental") {
      ...
      if (empty($_GET['url'])) $mult=1000; else $mult=1;      
    }
