...
if($action && $id)
{
	switch($action)
	{
      ...
      if ($subdivision_id!=NULL) {
		    ...
		    if($result)
		    {
			    unlink($_SERVER['DOCUMENT_ROOT'] . '/files/subdivisions_areas/documents/'.$filename);
			    $update_status = "deleted_document";
      ...
      if ($cities_id!=NULL) {
        ...
        if($result)
        {
          unlink($_SERVER['DOCUMENT_ROOT'] . '/files/cities/documents/'.$filename);
          $update_status = "deleted_document";
