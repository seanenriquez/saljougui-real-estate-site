...
function getAllSubdivCounts ($city) {
 ...
 echo "BEGIN processing subdivision counts for $city..."."\n\n";
  ...
  while ($row_sub = mysql_fetch_assoc($subdiv)) {
    ...
    $sql= "UPDATE community_area_information set current_hits = $row_counts[sd_count] where city = '".trim($row['city'])."' and subdivision_area_name = '".trim($row_sub['subdivision_area_name'])."'"; 
    ...
    // update table with counts
    $sql= "UPDATE community_area_information set current_rental_hits = $row_counts[sd_count] where city = '".trim($row['city'])."' and subdivision_area_name = '".trim($row_sub['subdivision_area_name'])."'"; 
    mysql_query($sql) or die(mysql_error() . $sql);
...
function getSaleCityCounts ($city) {
